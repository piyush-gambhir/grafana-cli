...
func newCmdLibraryElementUpdate(f *cmdutil.Factory) *cobra.Command {
	var file string
...
Examples:
  ...
  grafana library-element update leUid123 -f updated-panel.json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var req client.LibraryElementUpdateRequest
			if err := cmdutil.UnmarshalInput(file, &req); err != nil {
				return err
			}

			result, err := c.UpdateLibraryElement(context.Background(), args[0], req)
			if err != nil {
				return err
			}

			if f.Resolved.Output == "table" {
				fmt.Fprintf(f.IOStreams.Out, "Library element updated: %s (UID: %s)\n", result.Result.Name, result.Result.UID)
				return nil
			}
