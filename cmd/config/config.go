...
func NewCmdConfig(f *cmdutil.Factory) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "View and modify the grafana CLI configuration file.",
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(cfg.Profiles) == 0 {
				fmt.Fprintln(f.IOStreams.Out, "No profiles configured. Run 'grafana login' to create one.")
				return nil
			}
